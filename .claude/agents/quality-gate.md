...
### Chrome DevTools MCP 자동 캡처
- Chrome DevTools MCP를 사용한 자동 캡처는 로컬 확인 단계에서 자동으로 실행
- **서버 기동**: 사용자가 요청한 경우에만 (quality-gate가 직접 서버를 시작하지 않음)
- **캡처 범위**: ui-spec.md에 정의된 각 화면의 주요 상태 (초기 로드, 인터랙션 후)
- **캡처 결과**: docs/tests/{feature}/{timestamp}.md에 추가

### 절차
1. 개발 서버 확인: `curl -s -o /dev/null -w "%{http_code}" http://localhost:3000`
   - 미실행 시 → 보고만 함 ("개발 서버 미실행으로 시각 검증 생략"). quality-gate가 직접 서버를 시작하지 않음
...
3. 각 화면 (초기 로드 상태):
   ...
   - 불일치 항목 기록
4. 각 화면 (인터랙션 후 상태):
   - ui-spec.md에 정의된 주요 인터랙션 수행
   - `take_screenshot` → 인터랙션 후 캡처
   - 기대 상태와 비교
5. 반응형 검증 (ui-spec.md에 반응형 명시된 경우):
